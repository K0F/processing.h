...
Compatibility helpers:
  - size(w,h) or size(w,h,title)
  - background(v) or background(r,g,b) or background(r,g,b,a)
  - setup() and draw() are declared as void returning
Requires raylib (with rlgl). Link with -lraylib -lm -lpthread -ldl (platform dependent).
...
static inline int size_with_title_wrapper(int w, int h, const char *title) {
    return size_with_title(w, h, title ? title : "processing");
}
